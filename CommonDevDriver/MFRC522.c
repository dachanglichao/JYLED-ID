...
//	delay_ms(1);
	...
	//等待接收数据完成     
	i = 100; //i根据时钟频率调整操作M1卡最大等待时间25ms     
	do      
	{  
			OSTimeDlyHMSM(0,0,0,1); //将任务挂起1S		
...
/*----------------------------------------------------------
TYPE A或B 发送MSndBuffer并接收数据(入口：发送长度)(返回0 is ok)
----------------------------------------------------------*/
...
char ISO7816_TRANSCEIVE(unsigned char SendLen, void * SendBuf, unsigned char * RcvLen, void * Rcvdata)
{
	...
	do
	{
		...
		MSndBuffer[0] |=0x08;  //包含CID
		MSndBuffer[++i] = 0x00;  //CID  ...为了与TF COS兼容
		...
		if ((MSndBuffer[0]&0x10)==0x00) break;//分段发送结束
		if (!status)
		{
			if ((MRcvBuffer[0]&0xf0)!=0xa0)//检查分段确认(R-block)
	...
	*RcvLen=0;
	...
	do
	{
		if (!status)
		{
			...
			else if(MRcvBuffer[0] && (MRcvBuffer[0]&0xc0)==0)//信息分组 
			{
				...
				if ((MRcvBuffer[0]&0xf0)==0) //最后一帧信息
					break;
				else if ((MRcvBuffer[0]&0xf0)==0x10)//有后续信息
				{
					MSndBuffer[0]=((MRcvBuffer[0]&0x0f)|0xa0)^0x01;//发送分段接收确认
					MSndBuffer[0] |=0x08;  //包含CID
					MSndBuffer[1] = 0x00;  //CID  ...为了与TF COS兼容
					status = TypeAB_PiccTRANSCEIVE(2);
					MInfo.nBytesReceived =MInfo.nBitsReceived>>3;
				}
			}
