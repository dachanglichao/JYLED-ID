...
void	ChgUlongToDispBuffer(ulong iii,uchar * ptr);
//显示控制管脚初始化
void Disp_Gpio(void)
{
	  CLK_OUT();
...
//	memset(DispBuffer+5,0,5);
	if(!CurrentConsumMoney&&ConsumMode!=CONSUM_RATION)
	{
		memcpy(DispBuffer,"\x50\x79\x77\x5e\x6e",5);//ready
			memset(DispBuffer+5,0,5);
	}
	else
		memcpy(DispBuffer,"\x76\x79\x38\x38\x3f",5);//hello
...
void	DISP_HARDEErrorSub(uchar Num)
{
	memcpy(DispBuffer,DispHardeErrorString+Num*16,10);
	LED_DispDatas_all(DispBuffer);
}
...
void	Disp_Ver(uchar Num)//显示版本号
{
	...
	else if (Num==1)
	{
		...
		DispBuffer[0]=Disp0_9String[0];//U
		DispBuffer[1]=Disp0_9String[0];
		sendver[0] = 0;
		sendver[1] = 0;
		i=VerString[9]-0x30;
		...
		DispBuffer[4]=Disp0_9String[i/10];	
		sendver[4] = i/10;
		...
		if(i>3)
		{
	 	...
	 	DispBuffer[6]=Disp0_9String[i];
		sendver[6] = i;
		i=VerString[5]-0x30;	
		DispBuffer[7]=Disp0_9String[i];
		sendver[7] = i;
		
		memcpy(VerString,__TIME__,sizeof (__TIME__));//编译日期
		DispBuffer[8]=Disp0_9String[VerString[0] - '0'];
		sendver[8] = VerString[0] - '0';
		DispBuffer[9]=Disp0_9String[VerString[1] - '0'];
		sendver[9] = VerString[1] - '0';
...
void	Disp_Device_Num(uchar Num)//显示版本号
{
	uchar	i;
	uchar	VerString[11];
	...
	DispBuffer[9]=0;
	{
		DispBuffer[0]=Disp0_9String[snedData[2] >> 4];//U
		DispBuffer[1]=Disp0_9String[snedData[2] & 0x0F];
		DispBuffer[2]=Disp0_9String[snedData[3] >> 4];//U
		DispBuffer[3]=Disp0_9String[snedData[3] & 0x0F];
		DispBuffer[4]=Disp0_9String[snedData[4] >> 4];//U
		DispBuffer[5]=Disp0_9String[snedData[4] & 0x0F];
		DispBuffer[6]=Disp0_9String[snedData[5] >> 4];//U
		DispBuffer[7]=Disp0_9String[snedData[5] & 0x0F];
		DispBuffer[8]=Disp0_9String[snedData[6] >> 4];//U
		DispBuffer[9]=Disp0_9String[snedData[6] & 0x0F];
...
void	DispSumConsumMoney(uchar Mode,uchar	Num,ulong  MoneyValue)
{
	...
	DispBuffer[0]=Disp0_9String[Purse_num];
	DispBuffer[1]=0x40;	
	...
	if ( !MoneyValue)
	{
		...
		if(ConsumMode!=CONSUM_RATION)
		{
			if(!Consum_Status)
			{
			  	memcpy(DispBuffer+2,"\x37\x79\x78",3);
			}
			else
				memcpy(DispBuffer+2,"\x37\x5c",2);
		} 
		...
		if (Mode==CONSUM_NUM)
		{
			...
			if(Num && Num_MulValue>1)
			{
				DispBuffer[LedX]=Disp0_9String[Num_MulValue%10];
				DispBuffer[LedX+1]=0x40;
			}
			
			DispBuffer[LedX+3]=Disp0_9String[Num/10];
			DispBuffer[LedX+4]=Disp0_9String[Num%10];	
		}
		else	
			DispBuffer[LedX+4]=Disp0_9String[0];

	}
	else
		ChgUlongToDispBuffer(MoneyValue,DispBuffer+LedX);
	//DispBuffer[10]=DispModeChar[ConsumMode];
					if (BatModeFlag) //
						DispBuffer[10]|=0x01;		
					else
...
void	Disp_Balance(uchar	* Balance)//显示余额
{
	...
	iii=((ulong)Balance[0])*0x10000+((ulong)Balance[1])*0x100+((ulong)Balance[2]);
  	if (MoneyMulValue==10)
		iii*=10;
	else if (MoneyMulValue==100)
		iii*=100;
	ChgUlongToLongDispBuffer(iii,DispBuffer);
	ChgUlongToDispBuffer(CurrentConsumMoney,DispBuffer+5);
	DispBuffer[10]=DispModeChar[ConsumMode];
  LED_DispDatas_all(DispBuffer);
}
...
void	Disp_No_Consume(void)//显示脱机下不允许脱机消费;
{
...
//ad by lc
void	DispSumConsumMoney_Add(ulong Money)
{
	memset(DispBuffer,0,11);
	DispBuffer[0]=Disp0_9String[Purse_num];
	DispBuffer[1]=0x40;	
	if(!Consum_Status)
	{
		...
		memcpy(DispBuffer+2,"\x37\x5c",2);
	ChgUlongToDispBuffer(Money,DispBuffer+5);
	LED_DispDatas_all(DispBuffer);
...
void	Disp_Clr_WhilteNum(void)//显示清空Pos;
{
	DispBuffer[0]=0x00;	
	DispBuffer[1]=0x39;//C
	DispBuffer[2]=0x38;//L
