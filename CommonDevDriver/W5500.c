...
unsigned char S1_Port[2];	//端口1的端口号(5000) 
unsigned char S1_DIP[4];	//端口1目的IP地址 
unsigned char S1_DPort[2];	//端口1目的端口号(6000) 

unsigned char S2_Port[2];	//端口2的端口号(5000) 
unsigned char S2_DIP[4];	//端口2目的IP地址 
unsigned char S2_DPort[2];	//端口2目的端口号(6000) 

unsigned char UDP_DIPR[4];	//UDP(广播)模式,目的主机IP地址
unsigned char UDP_DPORT[2];	//UDP(广播)模式,目的主机端口号
unsigned char UDP1_DPORT[2];	//UDP(广播)模式,目的主机端口号
...
void UDP_Write_SOCK_Data_Buffer(SOCKET s, unsigned char *dat_ptr, unsigned short size)
{
	...
	if((Read_W5500_SOCK_1Byte(s,Sn_MR)&0x0f) != SOCK_UDP)//如果Socket打开失败
	{		
		Write_W5500_SOCK_4Byte(s, Sn_DIPR, dataIP);//设置目的主机IP  		
		Write_W5500_SOCK_2Byte(s, Sn_DPORTR, 9);//设置目的主机端口号				
	}
...
void Socket_Init(SOCKET s)
{
	...
	switch(s)
	{
		...
		case 0x02:
			break;
		case 0x03:
			Write_W5500_SOCK_2Byte(3, Sn_DPORTR, 67);//设置目的主机端口号		
		...
		case 0x04:
			...
			break;
		case 0x05:
			/* set Socket n Port Number 这里为：8000*/
		...
		case 0x06:
			...
			Write_W5500_SOCK_4Byte(6, Sn_DIPR, filesvrIP);//设置目的主机IP  		
			Write_W5500_SOCK_2Byte(6, Sn_DPORTR, filesvrPOTY);//设置目的主机端口号		
//		filesvrport++;
//		if(filesvrport > 5000) filesvrport = 436;
		...
		case 0x07:
			...
			Write_W5500_SOCK_4Byte(7, Sn_DIPR, svraddrip);//设置目的主机IP  		
			Write_W5500_SOCK_2Byte(7, Sn_DPORTR, svraddrport);//设置目的主机端口号		
			Socket_Connect(7);
			break;
	}
}
...
IntDispose:
	...
	if((i & S0_INT) == S0_INT)//Socket0事件处理 
	{
		...
		{
			j=Read_W5500_SOCK_1Byte(1,Sn_IR);//读取Socket0中断标志寄存器
			...
			{
				Write_W5500_SOCK_1Byte(1,Sn_CR,CLOSE);// 关闭端口,等待重新打开连接 
				S1_State=1;//网络连接状态0x00,端口连接失败
			}
		}
		
		if((i & S2_INT) == S2_INT)//Socket1事件处理 
		{
